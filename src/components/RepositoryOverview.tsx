    ...
    <section className="rounded-3xl border border-slate-800/70 bg-slate-950/70 p-6 shadow-xl shadow-slate-950/40">
      <div className="flex flex-wrap items-start justify-between gap-6">
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Repository overview</p>
          <h2 className="text-xl font-semibold text-slate-100">{repository.repoName}</h2>
          <p className="text-xs text-slate-400">{totalRuns} recorded run{totalRuns === 1 ? '' : 's'} for this workspace.</p>
        </div>
...
const toneStyles: Record<NonNullable<RepositoryStatProps['tone']>, string> = {
  default: 'border-slate-800/70 bg-slate-900/60 text-slate-100',
...
const iconToneStyles: Record<NonNullable<RepositoryStatProps['tone']>, string> = {
  default: 'bg-slate-900/50 text-slate-300',
